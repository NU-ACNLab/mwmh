...
qual_df[qual_df$task == 'rest', 'prop_trs_kept'] <- qual_df[qual_df$task == 'rest', 'num_trs_kept']/1110
qual_df[qual_df$task == 'avoid', 'prop_trs_kept'] <- qual_df[qual_df$task == 'avoid', 'num_trs_kept']/300
qual_df[qual_df$task == 'faces', 'prop_trs_kept'] <- qual_df[qual_df$task == 'faces', 'num_trs_kept']/200
...
qual_df[qual_df$task == 'rest', 'minutes_remaining'] <- qual_df[qual_df$task == 'rest', 'prop_trs_kept']*minutes_rest
qual_df[qual_df$task == 'avoid', 'minutes_remaining'] <- qual_df[qual_df$task == 'avoid', 'prop_trs_kept']*minutes_avoid
qual_df[qual_df$task == 'faces', 'minutes_remaining'] <- qual_df[qual_df$task == 'faces', 'prop_trs_kept']*minutes_faces

qual_df$exclude <- ifelse(qual_df$minutes_remaining < 5, 1, 0)

qual_df <- qual_df[, c('subid', 'sesid', 'task', 'prop_trs_kept', 'minutes_remaining', 'exclude')]

write.csv(qual_df, paste0('/projects/b1108/studies/mwmh/data/processed/neuroimaging/tabulated/exclusions_', Sys.Date(), '.csv'), row.names=FALSE)
