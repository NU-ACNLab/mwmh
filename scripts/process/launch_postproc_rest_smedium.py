...
for subdir in subdirs:
    ...
    ses1_outfile = os.path.exists(indir + sub + '/ses-1/func/' + sub + '_ses-1_task-rest_space-fsLR_desc-smepostproc_bold.dscalar.nii')
    if ses1_rest and not ses1_outfile:
        ses = 'ses-1'
        cmd = ['Rscript /projects/b1108/studies/mwmh/scripts/process/postproc_rest_smedium.R -s ', subid, ' -e 1']
        postproc_rest_smedium_script = launchdir+sub+'_'+ses+'_postproc_rest_smedium_run.sh'
        os.system('cat /projects/b1108/studies/mwmh/scripts/process/sbatchinfo_20min_10G_general.sh > '+postproc_rest_smedium_script)
        os.system('echo '+' '.join(cmd)+' >> '+postproc_rest_smedium_script)
