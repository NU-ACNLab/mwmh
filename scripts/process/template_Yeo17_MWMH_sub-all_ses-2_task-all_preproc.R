...
for (j in 1:nrow(temp_subjs)) { 
  ...
  path <- c(system(paste0('find ', indir, 'surf/sub-', subid, '/ses-', 
            sesid, '/func/ ', '-name "*_space-fsLR_desc-preproc_smoothed.dscalar.nii"'), intern=TRUE))
  paths <- c(paths, path)
}

print('Estimate template.') #How can I include motion regressors? Censoring? Detrending? Low pass filter?
...
saveRDS(temp, paste0(outdir, 'temp_sub-all_ses-2_task-all_preproc.rds'))
