...
for subdir in subdirs:
    ...
    ses1_outfile = os.path.exists(indir + sub + '/ses-1/func/' + sub + '_ses-1_task-rest_space-fsLR_desc-maxpostproc_bold.dscalar.nii')
    if ses1_rest and not ses1_outfile:
        ses = 'ses-1'
        cmd = ['Rscript /projects/b1108/studies/mwmh/scripts/process/postproc_rest_maximum.R -s ', subid, ' -e 1']
        postproc_rest_maximum_script = launchdir+sub+'_'+ses+'_postproc_rest_maximum_run.sh'
        os.system('cat /projects/b1108/studies/mwmh/scripts/process/sbatchinfo_20min_10G_general.sh > '+postproc_rest_maximum_script)
    ...
    if ses2_rest and not ses2_outfile:
        ses = 'ses-2'
        cmd = ['Rscript /projects/b1108/studies/mwmh/scripts/process/postproc_rest_maximum.R -s ', subid, ' -e 2']
        postproc_rest_maximum_script = launchdir+sub+'_'+ses+'_postproc_rest_maximum_run.sh'
        os.system('cat /projects/b1108/studies/mwmh/scripts/process/sbatchinfo_20min_10G_general.sh > '+postproc_rest_maximum_script)
        os.system('echo '+' '.join(cmd)+' >> '+postproc_rest_maximum_script)
        os.system('chmod +x '+postproc_rest_maximum_script)
