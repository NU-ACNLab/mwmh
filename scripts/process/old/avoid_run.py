...
def get_parser():
    ...
    parser = ArgumentParser(description='Avoid Analysis Workflow',
    ...
    parser.add_argument(
        ...
        help='the root folder of a derivatives set generated with fMRIPrep '
             '(sub-XXXXX folders should be found at the top level in this folder).') #derivatives_dir = '/Users/flutist4129/Documents/Northwestern/studies/mwmh/data/processed/neuroimaging/fmriprep/'
    parser.add_argument('output_dir', action='store', type=Path,
                        help='the output path for the outcomes of preprocessing and visual '
                             'reports') # output_dir = '/Users/flutist4129/Documents/Northwestern/studies/mwmh/data/processed/neuroimaging/firstlevel/'
    parser.add_argument('analysis_level', choices=['participant', 'group'], nargs='+',
                        help='processing stage to be run, "participant" means individual analysis '
                             'and "group" is second level analysis.') # analysis_level = 'participant'
    ...
    g_bids.add_argument('--participant-label', action='store', type=str,
                        nargs='*', help='process only particular subjects') #participant_label = 'MWMH378'
    g_bids.add_argument('--session-label', action='store', type=str,
                        nargs='*', help='process only particular subjects') #session_label = 1
    g_bids.add_argument('--task', action='store', type=str, nargs='*',
                        help='select a specific task to be processed') #task = 'avoid'
    ...
    g_bids.add_argument('--space', action='store', choices=get_tpl_list() + ['T1w', 'template'],
                        help='select a specific space to be processed') #space = 'MNI152NLin6Asym'
    g_bids.add_argument('--bids-dir', action='store', type=Path,
                        help='point to the BIDS root of the dataset from which the derivatives '
                             ...
                             '(i.e. ``BIDS_root/derivatives``).') #bids_dir = '/Users/flutist4129/Documents/Northwestern/studies/mwmh/data/raw/neuroimaging/bids/'
...
def main():
    ...
    if opts.session_label:
        query['session'] = '|'.join(str(opts.session_label))
    if opts.run:
    ...
    # sub-MWMH378_ses-1_task-avoid_space-MNI152NLin6Asym_desc-preproc_bold.nii.gz
    prepped_bold = layout.get(**query, invalid_filters='allow') #doesn't like desc
    if not prepped_bold:
        print('No preprocessed files found under the given derivatives '
              'folder "%s".' % derivatives_dir, file=sys.stderr) #saying the file doesn't exist...

    # TMP:
    prepped_bold = ''.join([derivatives_dir, 'sub-', participant_label, '/ses-',
                        str(session_label), '/sub-', participant_label, '_ses-',
                        str(session_label), '_task-', task, '_space-', space,
