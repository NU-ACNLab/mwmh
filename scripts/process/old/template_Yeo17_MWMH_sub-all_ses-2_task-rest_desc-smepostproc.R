...
save(list=ls(), file=paste0(outdir, 'template_Yeo17_MWMH_sub-all_ses-2_task-rest_desc-smepostproc_args.rda'))

temp_subjs <- read.csv(paste0(indir, 'tabulated/temp_subjs_sub-all_ses-2_task-rest_desc-smepostproc.csv'))
...
print('Get the paths to the smeimally postprocessed resting state data.')
...
for (j in 1:nrow(temp_subjs)) { 
  ...
  path <- c(system(paste0('find ', indir, 'surf/sub-', subid, '/ses-', 
            sesid, '/func/ ', '-name "*_space-fsLR_desc-smepostproc_bold.dscalar.nii"'), intern=TRUE))
  paths <- c(paths, path)
}
...
saveRDS(temp, paste0(outdir, 'temp_sub-all_ses-2_task-rest_desc-smepostproc.rds'))


#plot(temp, idx=1:17, fname=paste0('/Users/flutist4129/Documents/Northwestern/studies/mwmh/plots/temp_smepostproc_', 1:17))