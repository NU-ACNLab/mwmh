...
def postproc_avoid_space_anat(sub, ses, funcindir, bidssesdir, sesoutdir):
    # Location of the pre-processed fMRI & mask
    flist = os.listdir(funcindir)
    file_avoid = os.path.join(funcindir, [x for x in flist if ('space-T1w_desc-preproc_bold.nii.gz' in x and 'task-avoid' in x)][0])
    file_avoid_mask = os.path.join(funcindir, [x for x in flist if ('space-T1w_desc-brain_mask.nii.gz' in x and 'task-avoid' in x)][0])
    ...
    param_avoid_file = open(os.path.join(funcindir, sub+'_'+ses+'_task-avoid_space-T1w_desc-preproc_bold.json'),)
    ...
    avoid_qual_df.to_csv(sesoutdir+'/'+sub+'_'+ses+'_task-avoid_quality.csv', index=False)
