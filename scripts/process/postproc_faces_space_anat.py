...
def postproc_faces_space_anat(sub, ses, funcindir, bidssesdir, sesoutdir):
    # Location of the pre-processed fMRI & mask
    flist = os.listdir(funcindir)
    file_faces = os.path.join(funcindir, [x for x in flist if ('space-T1w_desc-preproc_bold.nii.gz' in x and 'task-faces' in x)][0])
    file_faces_mask = os.path.join(funcindir, [x for x in flist if ('space-T1w_desc-brain_mask.nii.gz' in x and 'task-faces' in x)][0])
    ...
    param_faces_file = open(os.path.join(funcindir, sub+'_'+ses+'_task-faces_space-T1w_desc-preproc_bold.json'),)
