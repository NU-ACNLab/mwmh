...
save(list=ls(), file=paste0(outdir, 'template_Yeo17_MWMH_sub-ses2_ses-rand_task-rest_desc-maxpostproc_args.rda'))

temp_subjs <- read.csv(paste0(indir, 'tabulated/temp_subjs_sub-ses2_ses-rand_task-rest_desc-maxpostproc.csv'))
...
for (j in 1:nrow(temp_subjs)) { 
  ...
  path <- c(system(paste0('find ', indir, 'surf/sub-', subid, '/ses-', 
            sesid, '/func/ ', '-name "*_space-fsLR_desc-maxpostproc_bold.dscalar.nii"'), intern=TRUE))
  paths <- c(paths, path)
}
...
saveRDS(temp, paste0(outdir, 'temp_sub-ses2_ses-rand_task-rest_desc-maxpostproc.rds'))


#plot(temp, idx=1:17, fname=paste0('/Users/flutist4129/Documents/Northwestern/studies/mwmh/plots/temp_maxpostproc_sub-ses2_ses-rand_', 1:17))