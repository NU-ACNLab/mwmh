...
def postproc_rest_space_anat(sub, ses, funcindir, bidssesdir, sesoutdir):
    ...
    file_rest_mask = os.path.join(funcindir, [x for x in flist if ('space-T1w_desc-brain_mask.nii.gz' in x and 'task-rest' in x)][0])
    ...
    param_rest_file = open(os.path.join(funcindir, sub+'_'+ses+'_task-rest_space-T1w_desc-preproc_bold.json'),)
    ...
    rest_cen2.to_filename(sesoutdir+'/'+sub+'_'+ses+'_task-rest_space-T1w_desc-postproc_bold.nii.gz')
